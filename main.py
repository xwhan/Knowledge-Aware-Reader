    ...
    for epoch in range(cfg['num_epoch']):
        try:
            ...
            for iteration in tqdm(range(train_data.num_data // cfg['batch_size'])):
                ...
                batch = train_data.get_batch(iteration, cfg['batch_size'], 0.0)
                loss, pred, _ = my_model(batch)
                pred = pred.data.cpu().numpy()
                acc, max_acc = cal_accuracy(pred, batch[-2])
            ...
            tf_logger.add_scalar('train_loss', sum(train_loss) / len(train_loss), epoch)
            tf_logger.add_scalar('avg_train_acc', sum(train_acc) / len(train_acc), epoch)
            # tf_logger.add_scalar('max_train_acc', sum(train_max_acc) / len(train_max_acc), epoch)
            ...
            tf_logger.add_scalar('eval_acc', eval_acc, epoch)
...
def inference(my_model, valid_data, entity2id, cfg, log_info=False):
    ...
    for iteration in tqdm(range(valid_data.num_data // test_batch_size + 1)):
        batch = valid_data.get_batch(iteration, test_batch_size, fact_dropout=0.0)
        loss, pred, pred_dist = my_model(batch)
        pred = pred.data.cpu().numpy()
        acc, max_acc = cal_accuracy(pred, batch[-2])
        if log_info: 
            output_pred_dist(pred_dist, batch[-2], id2entity, iteration * test_batch_size, valid_data, f_pred)
